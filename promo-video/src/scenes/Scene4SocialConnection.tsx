...
const ModalRow: React.FC<ModalRowProps> = ({
  ...
  <div style={{
    ...
    boxShadow: active
      ? '0 0 0 2px rgba(13,121,117,0.55), 0 8px 32px rgba(13,121,117,0.18)'
      : '0 4px 16px rgba(0,0,0,0.05)',
    border: active ? '1.5px solid rgba(20,184,166,0.55)' : '1px solid #f1f5f9',
    ...
    {/* Text */}
    <div style={{ flex: 1 }}>
      <p style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 17, fontWeight: 700, color: '#1e293b', margin: 0 }}>
        {title}
      </p>
    ...
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
      stroke={active ? '#0d7975' : '#cbd5e1'} strokeWidth="2.5" strokeLinecap="round">
...
const JoinIcon = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
    stroke="#0d7975" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
const GroupsPage: React.FC = () => (
  <div style={{ position: 'absolute', inset: 0, backgroundColor: '#f6f7f9', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
    {/* Header */}
    <div style={{ background: 'linear-gradient(145deg,#62b5b3 0%,#309896 40%,#0d7975 100%)', height: 178, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <h2 style={{ position: 'absolute', left: 22, bottom: 22, fontFamily: 'Inter,system-ui,sans-serif', fontSize: 38, fontWeight: 800, color: 'white', margin: 0 }}>Groups</h2>
    </div>
    {/* Search */}
    <div style={{ padding: '20px 18px 16px', background: '#f6f7f9' }}>
      <div style={{ background: 'white', borderRadius: 18, padding: '13px 18px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#258280" strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      ...
      {[
        ...
        <div key={i} style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16, borderBottom: '1px solid #f1f3f5' }}>
          <div style={{ width: 54, height: 54, borderRadius: 27, background: g.isIcon ? '#e8f3f2' : g.color, display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: 0 }}>
            {g.isIcon && <svg width="26" height="26" viewBox="0 0 24 24" fill="#0d7975"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 17, fontWeight: 700, color: '#1e293b' }}>{g.name}</span>
              {g.time && <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 12, color: i === 0 ? '#0d7975' : '#94a3b8', fontWeight: 600 }}>{g.time}</span>}
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {g.badge > 0 && <div style={{ minWidth: 22, height: 22, borderRadius: 11, background: '#0d7975', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '0 5px' }}><span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 11, fontWeight: 700, color: 'white' }}>{g.badge}</span></div>}
            </div>
          </div>
        </div>
      ))}
    ...
    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 78, background: 'white', display: 'flex', justifyContent: 'space-around', alignItems: 'center', paddingBottom: 16, borderTop: '1px solid rgba(0,0,0,0.05)' }}>
      {(['Home','Groups','Favorite','Profile'] as const).map((lbl, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
          ...
          {i===1 && <svg width="24" height="24" viewBox="0 0 24 24" fill="#0d7975"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>}
          ...
          <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 12, fontWeight: i===1?600:400, color: i===1?'#0d7975':'#94a3b8' }}>{lbl}</span>
        </div>
      ))}
    </div>
  </div>
...
const AnimatedFAB: React.FC<{ frame: number, fabScale: number, fabGlow: number, modalEnter: number }> = ({ frame, fabScale, fabGlow, modalEnter }) => {
  ...
  return (
    <div style={{
      ...
      boxShadow: `0 6px 18px rgba(${r},${g},${b},${shadowAlpha}), 0 0 ${currentGlow*28}px ${currentGlow*12}px rgba(13,121,117,${(currentGlow*0.7).toFixed(2)})`,
      zIndex: 20,
    }}>
  ...
  <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 22, fontWeight: 800, color: '#1e293b' }}>Groups</span>
...
const DarkModalRow: React.FC<DarkRowProps> = ({
  ...
  <div style={{
    ...
    /* Dark fill */
    background: active
      ? 'linear-gradient(135deg, rgba(13,121,117,0.25), rgba(20,184,166,0.12))'
      : 'rgba(255,255,255,0.04)',
    border: active
      ? '1.5px solid rgba(20,184,166,0.55)'
      : '1.5px solid rgba(255,255,255,0.08)',
    boxShadow: active
      ? '0 8px 32px rgba(13,121,117,0.22)'
      : 'none',
    ...
    {/* Chevron */}
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
      stroke={active ? '#5eead4' : 'rgba(255,255,255,0.18)'} strokeWidth="2.5" strokeLinecap="round">
      <path d="M9 18l6-6-6-6"/>
    </svg>
  </div>
);
...
const JoinGroupFlow: React.FC<{ lf: number }> = ({ lf }) => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#fdfdfd', display: 'flex', flexDirection: 'column' }}>
      {/* Teal curved header */}
      <div style={{ background: 'linear-gradient(160deg,#1f918e 0%,#0d7975 100%)', padding: '60px 24px 70px', position: 'relative', borderRadius: '0 0 45% 15%', boxShadow: '0 4px 10px rgba(13,121,117,0.2)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <h2 style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 26, fontWeight: 700, color: 'white', margin: 0 }}>Join Group</h2>
        </div>
      </div>
      {/* Tab switcher */}
      <div style={{ padding: '0 24px', marginTop: -32, position: 'relative', zIndex: 10, flexShrink: 0 }}>
        <div style={{ background: 'white', borderRadius: 28, padding: 6, display: 'flex', position: 'relative', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
          <div style={{ position: 'absolute', top: 6, bottom: 6, left: `calc(${tabP * 50}% + 6px)`, width: 'calc(50% - 12px)', background: 'linear-gradient(135deg,#1f918e,#0d7975)', borderRadius: 22, boxShadow: '0 4px 12px rgba(13,121,117,0.3)' }} />
          <div style={{ flex: 1, padding: '16px 0', textAlign: 'center', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 16, fontWeight: 700, color: tabP < 0.5 ? 'white' : '#94a3b8' }}>Scan QR</span>
          </div>
          <div style={{ flex: 1, padding: '16px 0', textAlign: 'center', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 16, fontWeight: 700, color: tabP > 0.5 ? 'white' : '#cbd5e1', letterSpacing: '0.05em' }}>***</span>
            <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 16, fontWeight: 700, color: tabP > 0.5 ? 'white' : '#94a3b8' }}>Enter Code</span>
          </div>
        </div>
      </div>
      {/* QR content */}
      <div style={{ opacity: 1 - tabP, position: 'absolute', left: 0, right: 0, top: 210, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px' }}>
        ...
        <h3 style={{ fontFamily: 'Inter', fontSize: 24, fontWeight: 800, color: '#1e293b', margin: '18px 0 6px' }}>Scan to Join</h3>
        ...
        <div style={{ marginTop: 18, border: '2.5px solid #14b8a6', borderRadius: 16, padding: '14px 0', width: '100%', textAlign: 'center', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: 'Inter', fontSize: 18, fontWeight: 700, color: '#14b8a6' }}>Toggle Flashlight</span>
        </div>
      </div>
      {/* Code content */}
      <div style={{ opacity: tabP, position: 'absolute', left: 0, right: 0, top: 210, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '32px 24px' }}>
        <div style={{ width: 100, height: 100, borderRadius: 50, background: '#eef2f5', display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: 20 }}>
          <svg width="46" height="46" viewBox="0 0 24 24" fill="none" stroke="#0d7975" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M18 3a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3 3 3 0 0 0 3-3 3 3 0 0 0-3-3H6a3 3 0 0 0-3 3 3 3 0 0 0 3 3 3 3 0 0 0 3-3V6a3 3 0 0 0-3-3 3 3 0 0 0-3 3 3 3 0 0 0 3 3h12a3 3 0 0 0 3-3 3 3 0 0 0-3-3z"/>
          </svg>
        </div>
        <h3 style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 26, fontWeight: 800, color: '#1e293b', margin: '0 0 8px' }}>Enter Group Code</h3>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 40 }}>
          {digits.map((d, i) => {
            ...
            return (
              <div key={i} style={{ width: 48, height: 60, borderRadius: 14, background: 'transparent', border: `1.5px solid ${filled ? '#0d7975' : '#e2e8f0'}`, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 28, fontWeight: 700, color: filled ? '#1e293b' : 'transparent' }}>{d}</span>
              </div>
            );
          })}
        </div>
        <div style={{ background: 'linear-gradient(90deg,#4dc0b5,#0d7975)', borderRadius: 20, padding: '16px 0', width: '100%', textAlign: 'center', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, boxShadow: '0 6px 20px rgba(13,121,117,0.3)' }}>
          <span style={{ fontFamily: 'Inter,system-ui,sans-serif', fontSize: 20, fontWeight: 700, color: 'white' }}>Join Group</span>
    ...
    <AbsoluteFill style={{ backgroundColor: '#050714', overflow: 'hidden', opacity: exitOp }}>
      ...
      {frame >= ISO_ENTER && frame < JOIN_START && (
        <div style={{
          ...
          <h1 style={{
            fontFamily: 'Inter,system-ui,sans-serif', fontSize: 36, fontWeight: 900,
