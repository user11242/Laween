...
const USER_DEFS = [
  ...
  {
    letter: 'W', name: 'You', color: '#006D77',
...
export const Scene10LiveTracking: React.FC = () => {
  ...
  const friends = [
    ...
    { rank: 4, name: 'User', dist: '5.5 km away', eta: '11', arrived: false, color: '#006D77' },
  ];
      ...
      <div style={{ transform: `scale(${phoneScale}) translateY(80px)` }}>
        <PhoneMockup scale={0.95} drift>
          ...
          <div style={{
            ...
            {/* ── DESTINATION PIN + arrival pulse ring ── */}
            <div style={{
              position: 'absolute', top: `${DEST.top}%`, left: `${DEST.left}%`,
              transform: 'translate(-50%, -100%)', zIndex: 20,
            }}>
              {/* Pulsing ring when someone arrives */}
              {(yazanArrived || sarahArrived) && (
                <div style={{
                  position: 'absolute', inset: -12, borderRadius: '50%',
                  border: `2px solid rgba(20,184,166,${0.3 + Math.sin(frame * 0.2) * 0.2})`,
            ...
            {computedMarkers.map((m) => (
              <div key={m.letter} style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 26, fontWeight: 800, color: 'white' }}>{m.letter}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="#2D3748">
                <path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5z"/>
              </svg>
              <h2 style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 18, fontWeight: 800, color: '#2D3748', margin: 0 }}>
              ...
              {friends.map((f, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ width: 40, display: 'flex', justifyContent: 'center' }}>
                    {f.arrived ? (
                      <div style={{ width: 34, height: 34, borderRadius: 17, background: '#006D77', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                      ...
                      <div style={{ width: 34, height: 34, borderRadius: 17, background: '#f1f5f9', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <span style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 15, fontWeight: 700, color: '#334155' }}>{f.rank}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ flex: 1, paddingLeft: 12 }}>
                    <p style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 17, fontWeight: 800, color: '#2D3748', margin: '0 0 4px', letterSpacing: '-0.01em' }}>{f.name}</p>
                  ...
                  <div>
                    {f.arrived ? (
                      <div style={{
                        background: '#e0f2fe', borderRadius: 16, padding: '8px 14px',
                        boxShadow: '0 0 10px rgba(0,109,119,0.15)',
                        animation: undefined,
                        // Pop scale using frame-based spring
                        transform: `scale(${
                          interpolate(
                            spring({ frame: Math.max(0, frame - (f.rank === 1 ? USER_DEFS[0].arriveFrame : USER_DEFS[1].arriveFrame)), fps, config: { damping: 10, stiffness: 200 } }),
                            [0, 1], [0.5, 1], { extrapolateRight: 'clamp' }
                          )
                        })`,
                      }}>
                        <span style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 11, fontWeight: 800, color: '#006D77', letterSpacing: '0.05em' }}>ARRIVED</span>
                      </div>
                    ) : (
                      <div style={{ background: '#fef3c7', borderRadius: 16, padding: '8px 16px', display: 'flex', alignItems: 'baseline', gap: 4 }}>
                        <span style={{ fontFamily: 'Outfit, system-ui, sans-serif', fontSize: 16, color: '#d97706', fontWeight: 800 }}>{f.eta}</span>
